#!/usr/bin/env sh
#
# Regenerate raster icon variants from the SVG sources.
#   - Cross-platform PNGs and Windows .ico work on Linux + macOS.
#   - The .icns step requires `iconutil`, which only ships on macOS.
...
# macOS .icns — only when iconutil is available
if command -v iconutil >/dev/null 2>&1; then
  ...
  echo "iconutil not found (macOS-only) — skipping icon.icns regeneration."
  echo "Run this script on a macOS machine before shipping a packaged build."
fi